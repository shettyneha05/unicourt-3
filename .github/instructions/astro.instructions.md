...
- Define a `Props` interface in frontmatter for every reusable component and
  document the component contract with a TSDoc/JSDoc comment. Describe each
  prop's purpose, accepted values, and whether it is optional.
...
- Comment why a component uses a non-obvious structure or accessibility
  behavior; do not add comments that restate the markup or class names.
- Keep `Props` documentation and other comments current whenever the component
  API or rendering behavior changes.
