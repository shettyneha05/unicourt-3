...
- Add a TSDoc/JSDoc comment to every exported function in `db/` and `src/lib/`.
  Each comment must state the function's purpose, describe every parameter with
  `@param` (including the injectable `db` argument), and describe the return
  value with `@returns`. Keep the explanation focused on intent and invariants,
  not a paraphrase of the implementation.
- Keep comments current with the code. Update or remove a comment in the same
  change when behavior or a constraint changes.
